...
class MNIST_Reduced(MNIST):
    def __init__(self, root, train=True, transform=None, download=False):
        super(MNIST_Reduced, self).__init__(root=root, train=train, transform=transform, download=download)
        self.data_filtered = []
        self.targets_filtered = []
        for data, target in zip(self.data, self.targets):
            if target in [0,1]:
                self.data_filtered.append(data)
                self.targets_filtered.append(target)
        self.data = self.data_filtered
        self.targets = self.targets_filtered
...
def get_dataset(dataset_name, root='../data', pair=True):
    if pair:
        if dataset_name=='cifar10':
            ...
            train_data = MNISTPair(root=root, train=True, transform=train_transform, download=True)
            memory_data = MNISTPair(root=root, train=True, transform=test_transform, download=True)
            test_data = MNISTPair(root=root, train=False, transform=test_transform, download=True)
        else:
        ...
        if dataset_name in ['cifar10', 'cifar10_true_label']:
            ...
            train_data =  MNIST_Reduced(root=root, train=True, transform=train_transform,download=True)
            memory_data = MNIST_Reduced(root=root, train=True, transform=test_transform, download=True)
            test_data =  MNIST_Reduced(root=root, train=False, transform=test_transform, download=True)
        else:
            raise Exception('Invalid dataset name')
